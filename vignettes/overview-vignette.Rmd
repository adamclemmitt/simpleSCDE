...
Mining data for SCDE analysis can be particularly difficult due to the variety of data types and sizes available for potential use. In order to accomodate as many scenarios as possible, 'simpleSCDE' includes methods for reading CSV, text, and excel files. Below are examples of how to use all three functions:
...
In the above examples, the imported data is being assigned to the values `import_csv`, `import_txt`, and `import_excel`. The provided import functions can be used without assigning their value to a user-defined variable, though the results of the import function will not be stored anywhere.
...
If you would like to see descriptions or parameters of any of the functions above, remember that `?` can be used in the console in front of a functions name to see these details and more about that specific function..
